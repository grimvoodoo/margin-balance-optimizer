...
async fn main() -> Result<()> {
    ...
    let last_asset_pairs_fetch = Arc::new(Mutex::new(std::time::Instant::now() - std::time::Duration::from_secs(3600)));
    let last_ohlc_fetch = Arc::new(Mutex::new(std::time::Instant::now() - std::time::Duration::from_secs(300)));
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Ok(mut file) = std::fs::OpenOptions::new()
                ...
                let _ = writeln!(file, "[{}] Fetching positions...", 
            ...
            match client_fast.get_open_positions().await {
                Ok(positions) => {
                    ...
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        ...
                        .append(true)
                        .open("/tmp/kraken_position_fetch.txt") {
                        let _ = writeln!(file, "[{}] Received {} positions", 
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            positions.len());
                    }
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Ok(mut file) = std::fs::OpenOptions::new()
                ...
                let _ = writeln!(file, "[{}] Fetching balances...", 
                    chrono::Utc::now().format("%H:%M:%S%.3f"));
            }
            ...
            match client_clone.get_balance().await {
                Ok(balances) => {
                    ...
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        ...
                        .append(true)
                        .open("/tmp/kraken_balance_fetch.txt") {
                        let _ = writeln!(file, "[{}] Received {} balances", 
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            balances.len());
                    }
            ...
            if !balance_assets.is_empty() && (should_fetch_pairs || asset_pair_map_clone.lock().unwrap().is_empty()) {
                if is_loading {
