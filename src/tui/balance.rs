...
pub fn render_balance(
    ...
    let mut balance_entries: Vec<BalanceEntry> = balances
        ...
        .filter_map(|(asset, balance_str)| {
            ...
            let is_fiat = asset.ends_with("GBP") || asset == "ZGBP" || asset == "GBP" 
                || asset.ends_with("EUR") || asset == "ZEUR" || asset == "EUR"
                || asset.ends_with("USD") || asset == "ZUSD" || asset == "USD";
            ...
            if is_fiat {
                ...
                if asset == "ZUSD" || asset == "USD" {
                    ...
                    if display_currency == "GBP" {
                        ...
                        current_price = ticker_data.get("GBPUSD")
                            .and_then(|t| t.c.first())
                            .and_then(|p| p.parse::<f64>().ok())
                    ...
                    if display_currency == "EUR" {
                        ...
                        // EUR -> USD -> display
                        current_price = ticker_data.get("EURUSD")
                            .and_then(|t| t.c.first())
                            .and_then(|p| p.parse::<f64>().ok())
    ...
    let total_value: f64 = balance_entries
        ...
        .filter(|e| {
            let asset = &e.asset;
            !(asset.ends_with("GBP") || asset == "ZGBP" || asset == "GBP" 
                || asset.ends_with("EUR") || asset == "ZEUR" || asset == "EUR"
                || asset.ends_with("USD") || asset == "ZUSD" || asset == "USD")
        })
        .map(|e| e.value_gbp)
    ...
    for entry in &mut balance_entries {
        let is_fiat = entry.asset.ends_with("GBP") || entry.asset == "ZGBP" || entry.asset == "GBP" 
            || entry.asset.ends_with("EUR") || entry.asset == "ZEUR" || entry.asset == "EUR"
            || entry.asset.ends_with("USD") || entry.asset == "ZUSD" || entry.asset == "USD";
    ...
    balance_entries.sort_by(|a, b| {
        b.value_gbp.partial_cmp(&a.value_gbp).unwrap_or(std::cmp::Ordering::Equal)
    });
    ...
    let rows: Vec<Row> = balance_entries
        ...
        .map(|entry| {
            ...
            let change_24h = if is_fiat {
                ...
                } else if entry.asset == "ZGBP" || entry.asset == "GBP" || entry.asset.ends_with("GBP") {
                    // GBP/USD change
            ...
            Row::new(vec![
                ...
                Cell::from(value_str),
                Cell::from(price_str),
                Cell::from(format!("{:+.2}%", change_24h)).style(
                    if change_24h > 0.0 {
